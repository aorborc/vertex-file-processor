...
export async function getDocument({ projectId, databaseId = "(default)", collection, docId }) {
  if (!projectId) throw new Error("getDocument missing projectId");
  if (!collection) throw new Error("getDocument missing collection");
  if (!docId) throw new Error("getDocument missing docId");
  const token = await getAccessToken();
  const endpoint = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/${encodeURIComponent(databaseId)}/documents/${collection}/${docId}`;
  ...
  const data = await res.json();
  return { name: data.name, fields: decodeValue({ mapValue: { fields: data.fields || {} } }) };
}
